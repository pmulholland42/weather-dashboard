...
export class Graph extends Component {
    ...
    componentDidUpdate() {
        ...
        if (typeof(yMax) === 'undefined') {
            yMax = Math.ceil(data.filter(element => {
                return Moment().isSame(element.time, timeScale);
            }).map(a => a.value).reduce((a, b) => {
                return Math.max(a, b);
            }, -Infinity));
        }
        if (typeof(yMin) === 'undefined') {
            yMin = Math.floor(data.filter(element => {
                return Moment().isSame(element.time, timeScale);
            }).map(a => a.value).reduce((a, b) => {
                return Math.min(a, b);
            }, Infinity));
        ...
        for (let hour = 0; hour <= 24; hour++) {
            if (hour % 3 === 0 ) {
                ...
                x -= 8;
                if (hour !== 0 && hour !== 24)
                    ctx.fillText(((hour > 12) ? (hour - 12) : hour)+ ":00", x + graphOffsetX - 10, height - 10);
            }
        }

        // Draw a vertical line marking the current time
        const lengthOfGraph = Moment.duration(1, timeScale);
        const startOfGraph = Moment().startOf("day");
        var currentTimeX = (Moment().diff(startOfGraph) / lengthOfGraph) * graphWidth;
        ...
        data.forEach(element => {
            ...
            let x = element.time.diff(startOfGraph) / lengthOfGraph;
            if (!lineStarted) {
                ctx.moveTo((x * graphWidth) + graphOffsetX, height - (y * graphHeight) - graphOffsetY);
                lineStarted = true;
            }
            else {
                ctx.lineTo((x * graphWidth) + graphOffsetX, height - (y * graphHeight) - graphOffsetY);
                if (element.time.isAfter(Moment())) {
                    ctx.fillText(element.value, (x * graphWidth) + graphOffsetX - 40, height - (y * graphHeight) - graphOffsetY);
                }
            }
        });
        ctx.stroke();
